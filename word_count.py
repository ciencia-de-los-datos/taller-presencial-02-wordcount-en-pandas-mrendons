...
def load_input(input_directory): #leer los archivos y volverlos a un solo dataframe
    ...
    #dataframes.append( df = pd.read_csv(filenames[0], sep="\t", header=None, names=["text"]))
    #df = pd.read_csv(filenames[0], sep="\t", header=None, names=["text"])#lee el archivo, lo convierte en un dataframe, heaber para el nombre de las columnas
...
def clean_text(dataframe): #limpia los datos
    """Text cleaning"""
    #
    # Elimine la puntuación y convierta el texto a minúsculas.
    #
    dataframe = dataframe.copy()
    dataframe["text"] = dataframe["text"].str.lower() #cambia mayusculas por minusculas
